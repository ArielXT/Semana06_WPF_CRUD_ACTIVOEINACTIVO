...
namespace Data
{
    public class DProducto
    {
        ...
        public List<Producto> ListarActivo(Producto producto)
        {
            SqlParameter[] parameters = null;
            string comandText = string.Empty;
            List<Producto> productos = null;
            ...
            try
            {
                comandText = "USP_GetProductoActivo";
                parameters = new SqlParameter[1];
                parameters[0] = new SqlParameter("@EsActivo", SqlDbType.Bit);
                parameters[0].Value = producto.EsActivo;
                productos = new List<Producto>();
        ...
        public List<Producto> ListarInactivo(Producto producto)
        {
            SqlParameter[] parameters = null;
            string comandText = string.Empty;
            List<Producto> productos = null;
                    ...
                    CommandType.StoredProcedure, parameters))
                {
                    while (reader.Read())
                    {
                        productos.Add(new Producto
                        {
                            ...
                            Nombre = reader["Nombre"] != null ? Convert.ToString(reader["Nombre"]) : String.Empty,
                            Precio = reader["Precio"] != null ? Convert.ToDecimal(reader["Precio"]) : 0,
                            EsActivo = reader["EsActivo"] != null ? Convert.ToBoolean(reader["EsActivo"]) : false,
                        });
                    }
                }
            }
